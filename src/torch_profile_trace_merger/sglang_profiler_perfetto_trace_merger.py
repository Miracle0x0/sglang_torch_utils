...
def main(
    ...
    if profile_id is None:
        pattern = re.compile(r"^perfetto-compatible-([0-9.]+)-.*$")
        profile_id = max(
            m.group(1)
    ...
    interesting_paths = sorted(
        ...
        [
            p
            for p in dir_data.glob("*.json.gz")
            if p.name.startswith("perfetto-compatible-" + profile_id)
        ],
        key=_get_tp_rank_of_path,
    )
    print(f"{interesting_paths=}")

    output_path = dir_data / f"merged-perfetto-compatible-{profile_id}.trace.json.gz"
